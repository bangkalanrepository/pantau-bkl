...
export default function App() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-100 lg:h-dvh lg:overflow-hidden">
      <Header onRefresh={refresh} isRefreshing={isRefreshing} lastUpdated={lastUpdated} />

      <div className="mx-auto w-full max-w-[1700px] shrink-0 px-3 pt-4 sm:px-4">
        <SummaryCards locations={LOCATIONS} dataMap={dataMap} isLoading={isLoading} />
      </div>
      ...
      <main className="mx-auto flex min-h-0 w-full max-w-[1700px] flex-1 flex-col gap-4 px-3 py-4 sm:px-4 lg:grid lg:grid-cols-[minmax(340px,400px)_minmax(0,1fr)]">
        ...
        <div className="hidden min-h-0 lg:block">
          {selectedEntry !== undefined ? (
            <LocationDetail entry={selectedEntry} isLoading={isLoading} onClose={handleClose} />
          ) : (
            <div className="grid h-full place-items-center rounded-xl border-2 border-dashed border-slate-200 bg-white/60 p-6 text-center">
              <div className="max-w-xs space-y-2 text-slate-400">
                <MousePointerClick className="mx-auto h-8 w-8" aria-hidden="true" />
                <p className="text-sm font-medium text-slate-600">Pilih penanda di peta</p>
                <p className="text-xs leading-relaxed">
                  Klik salah satu titik lokasi untuk melihat detail cuaca, kualitas udara,
        ...
        >
          <MapView locations={LOCATIONS} dataMap={dataMap} selectedId={selectedId} onSelect={handleSelect} />
        </section>
